...
exports.register = async (req, res) => {
  const { first_name, last_name, phone_number, email, profile_photo, password, user_type } = req.body;
  ...
  try {
    ...
    // Create a new user
    const newUser = await User.createUser({ uid, first_name, last_name, phone_number, email, profile_photo, password: hashedPassword, user_type });

    const payload = { id: newUser.insertId, email: newUser.email, user_type: newUser.user_type, status: newUser.status };
...
exports.login = async (req, res) => {
  ...
  try {
    ...
    // Generate tokens
    const payload = { id: user.id, email: user.email, user_type: user.user_type, status: user.status };
    const accessToken = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '1h' });
...
exports.refreshToken = async (req, res) => {
  ...
  try {
    ...
    const accessToken = jwt.sign({ id: user.id, email: user.email, user_type: user.user_type }, process.env.JWT_SECRET, { expiresIn: '1h' });
